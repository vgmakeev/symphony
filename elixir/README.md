...
Symphony checks the final turn status in `turn/completed`. A failed or interrupted
Codex turn fails the worker run and enters retry backoff; it is not treated as
completed work.
...
return the Economic OS validation details to Codex, and weak goals stay
...
- Supported `codex.approval_policy` values depend on the targeted Codex app-server version. In the current local Codex schema, string values include `untrusted`, `on-request`, and `never`; granular policy uses the externally tagged `{"granular": {...}}` object form.
...
- `agent.max_turns` caps how many back-to-back Codex turns Symphony will run in a single agent
  ...
  Exhausting the limit while the issue remains active fails the run and uses failure backoff.
