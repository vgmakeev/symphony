defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server rejects a completed notification whose turn failed" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-failed-turn-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      while IFS= read -r _line; do
        ...
        case "$count" in
          ...
          4)
            ...
            printf '%s\\n' '{"method":"turn/completed","params":{"turn":{"id":"turn-90","status":"failed","error":{"message":"Your access token could not be refreshed."}}}}'
            exit 0
            ;;
          *) exit 0 ;;
        esac
      ...
      issue = %Issue{
        id: "issue-failed-turn",
        identifier: "MT-90",
        title: "Failed Codex turn",
        description: "A failed turn must not complete the agent run",
        state: "In Progress",
      ...
      assert {:error, {:turn_failed, params}} =
               AppServer.run(workspace, "Review failed turn", issue)

      assert get_in(params, ["turn", "error", "message"]) ==
